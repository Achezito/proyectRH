...
export default function LoginPage() {
 ...
 const handleLogin = async () => {
  ...
  try {
    const res = await fetch("http://192.168.1.84:5000/auth/login", {
      method: "POST",
    ...
    if (!res.ok) {
      if (res.status === 403) {
        if (data.error.includes("pendiente")) {
          ...
          setError("Tu cuenta está en revisión. Espera la aprobación del administrador.");
          return;
        }
...
<View style={styles.cardContent}>
  {error ? (
    <View style={[
      styles.alert,
      error.includes("revisión") && { backgroundColor: "#ecfdf5", borderColor: "#a7f3d0" }
    ]}>
      <AlertCircle size={16} color={error.includes("revisión") ? "#059669" : "#dc2626"} />
      <Text style={[
        styles.alertText,
